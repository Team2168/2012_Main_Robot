 ...
 * <br><br>
 * PIDPosition posController = new PIDPosition("DriveTrain Position Controller", P, I, D, leftEncoder, period); //launch the PID thread <br>
 * posController.Enable(); //start the PID thread<br>
 * 
 * 
 * <br><br>
 ...
 * @param P - type double which represents Proportional Gain for the Position Controller
 * @param I - type double which represents Integral Gain for the Position Controller
 * @param D - type double which represents Derivative Gain for the Position Controller
 ...
 */
			...
			Encoder currentPos, long period)
	{
		...
		//at Position size
		this.atPosition = new double[SIZE];
		this.count=0;
		...
		this.executor.schedule(new PIDPositionTask(this), 0L, this.period);
	 ...
	 *
	 * @param name - type String used to identify this PID Instance and thread i.e "LeftDrivePID"
	 * @param pUp - type double which represents Proportional Gain for the Position Controller to use when the Error is greater than zero.
	 * @param iUp - type double which represents Integral Gain for the Position Controller to use when the Error is greater than zero.
	 * @param dUp - type double which represents Derivative Gain for the Position Controller to use when the Error is greater than zero.
	 * @param pDown - type double which represents Proportional Gain for the Position Controller to use when the Error is less than zero.
	 * @param iDown - type double which represents Integral Gain for the Position Controller to use when the Error is less than zero.
	 * @param dDown - type double which represents Derivative Gain for the Position Controller to use when the Error is less than zero.
	...
	/**
	 * @return the encoder rate which represents the current Position of the device being controllerd
	 */
	...
	public synchronized double getEncoderDistance()
	{
		...
		Double encVal = new Double(encoder.getDistance());
	...
	private void isFinished()
	{
		...
		if (count==this.atPosition.length)
			count=0;
		...
		int inRange=0;
		for(int i=0,j=1; j<atPosition.length; i++,j++)
		{
			if (Math.abs(atPosition[j]) < Math.abs(atPosition[i]+atPosition[i]*percent) && Math.abs(atPosition[j]) > Math.abs(atPosition[i]-atPosition[i]*percent))
			 inRange++;
			else
				inRange=0;
		}
		if (inRange==atPosition.length-1)
			isFinished=true;
		else
			isFinished=false;
	...
	private void debug()
	{
		...
		SmartDashboard.putDouble(this.name+"_Encoder Distance", this.cp);
	 ...
	 * method should only be called from a PIDPositionTask object which runs this
	...
	private synchronized void calculate()
	{
		...
		if (enable)
		{
			...
			cp = encoder.getDistance();// cp is in units distance per second (i.e
									// inches/sec)
			//System.out.println(cp);
			//SmartDashboard.putDouble(name + "_Distance:", cp);
			//System.out.println("I am a thread" + name + " and execution # " + count++ + " and clock is " + executionTime + " encoder rate: " + encoder.getDistance());
	 ...
	 * 
	 */
	private class PIDPositionTask extends TimerTask
	{
		// internal PIDPosition object to run in new thread
		private PIDPosition posController;


		/**
		 * constructor for the private class PIDPositionTask, which will be used to
		 ...
		 */
		private PIDPositionTask(PIDPosition controller)
		{
